...
public class Test_INTEGER extends TestBase {
	...
	public void test() throws Exception {
		...
		for (byte i = 0; i < ROWS; i++) {
			...
			sql3.add(sql3_obj);
			Assert.assertTrue(sql3_obj.cShort == (i + 1));
			Assert.assertTrue(sql3_obj.attr1 == (i + 1));
			Assert.assertNull(sql3_obj.attr2);
			sql4.add(sql4_obj);
			Assert.assertTrue(sql4_obj.cByte == (i + 1));
			Assert.assertTrue(sql4_obj.attr1 == (i + 1));
			Assert.assertNull(sql4_obj.attr2);
		}

		for (byte i = 0; i < ROWS; i++) {
			session.persist(sql1_obj = new PK_INTEGER1((long) (i + 1)));
			Assert.assertNotNull(sql1_obj.cLong);
			Assert.assertNull(sql1_obj.attr2);
			session.persist(sql2_obj = new PK_INTEGER2(i + 1));
			Assert.assertNotNull(sql2_obj.cInteger);
			Assert.assertNull(sql2_obj.attr2);
			session.persist(sql3_obj = new PK_INTEGER3((short) (i + 1)));
			Assert.assertNotNull(sql3_obj.cShort);
			Assert.assertNull(sql3_obj.attr2);
			session.persist(sql4_obj = new PK_INTEGER4((byte) (i + 1)));
			Assert.assertNotNull(sql4_obj.cByte);
			Assert.assertNull(sql4_obj.attr2);
